...
def main():
    ...
    while count < num_inputs:
        ...
        # 调用diffusion_256进行fine粒度的采样
        sample_fine, internal_list_fine = diffusion_256.p_sample_loop(
            model_256,
        ...
        for i in range(args.batch_size):
            ...
            os.makedirs(os.path.join(logger.get_dir(), "internalImg_coarse", str(count + i)), exist_ok=True)
            os.makedirs(os.path.join(logger.get_dir(), "internalImg_fine", str(count + i)), exist_ok=True)
            ...
            utils.save_image(gtImg, out_gtImg_path, nrow=1, normalize=True)
            utils.save_image(inputImg, out_inputImg_path, nrow=1, normalize=True)
            utils.save_image(sampledImg, out_sampledImg_path, nrow=1, normalize=True)
            utils.save_image(outImg, out_outImg_path, nrow=1, normalize=True)
